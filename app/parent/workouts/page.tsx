...
export default function WorkoutsPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from("workouts")
      .select("id, title, subtitle, sport_tag, difficulty, is_shared, assigned_child_id, exercises")
    ...
    <main className="min-h-screen px-6 py-8 max-w-md mx-auto">
      ...
      {workouts.map((w) => (
        <div key={w.id} className="bg-white rounded-2xl p-4 shadow mb-3">
          ...
          {w.assigned_child_id && (
            <span className="inline-block text-[9px] font-bold text-grass ml-2">🎯 Assigned to a coached kid</span>
          )}
